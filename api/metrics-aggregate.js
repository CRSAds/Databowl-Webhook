// Snelle metrics feed (zelfde bron): sommeer per dag -> affiliate -> offer
...
function clampRange(date_from, date_to) {
  ...
  return { date_from: cf.toISOString().slice(0,10), date_to: ct.toISOString().slice(0,10) };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Zelfde boom als dashboard-aggregate (handig voor UI-hergebruik)
    ...
    for (const r of rows || []) {
      ...
      if (!d) { d = { key: dayKey, label: nlDate(dayKey), leads:0, cost:0, children:new Map() }; dayMap.set(dayKey, d); }
      let a = d.children.get(affKey);
      if (!a) { a = { key: affKey, label: affKey || '—', leads:0, cost:0, children:new Map() }; d.children.set(affKey, a); }
      let o = a.children.get(offKey);
      if (!o) { o = { key: offKey, label: offKey || '—', leads:0, cost:0, campaign_id: r.campaign_id || null }; a.children.set(offKey, o); }

      o.leads += leads;  o.cost += cost;
      a.leads += leads;  a.cost += cost;
      d.leads += leads;  d.cost += cost;
    }
