...
// - Schrijft 1) events_staging (upsert op event_key) en 2) lead_uniques_day_grp (insert ignore)
// - Bewaart cursor in tabel 'sync_state' (id='directus-events')
...
async function fetchDirectusPage({ lastCreatedAt, page = 1 }) {
  ...
  // filter: nieuwere rows én campagne 925 uitsluiten
  const filter = {
    _and: [
...
// 2) dedupe insert (ignore duplicates on PK composite)
async function upsertDedupe(rows) {
  ...
  // Gebruik insert with onConflict + ignoreDuplicates (we willen NIET updaten)
  const { error } = await sb
...
export default async function handler(req, res) {
  ...
  try {
    ...
    while (true) {
      ...
      page += 1;
      // mini-pauze om Directus te ontzien
