...
async function updateLastSynced(client, ts) {
  await client.from("sync_runs").insert({ last_synced_at: ts });
}
...
async function runEtlSync(since) {
  ...
  while (true) {
    ...
    console.log(`[sync] Run: ${url.toString()}`);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const since = lastSynced
      ? new Date(new Date(lastSynced).getTime() - 10 * 60 * 1000).toISOString() // overlap 10 min
      : startOfTodayISO();
    ...
    return res.status(200).json({ ok: true, since, finished_at: now, synced: result.synced || 0 });
  } catch (err) {
