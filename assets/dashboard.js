  ...
  function mapTreeToRows(tree) {
    return (tree || []).map(n => ({
      ...
      children: (n.children || []).map(a => ({
        key: a.key, label: a.label, leads: Number(a.leads||0), cost: Number(a.cost||0),
        children: (a.children || []).map(o => ({
          key: o.key, label: o.label, leads: Number(o.leads||0), cost: Number(o.cost||0), campaign_id: o.campaign_id ?? null
        }))
      }))
    })).sort(byISO);
  }
